...
make check                      # everything a commit must pass, ~20s
                                # (a pre-commit hook runs it; make hooks enables it)
make test                       # the suite alone — hermetic, no API key needed
...
**The status column is the queue.** No broker. The worker picks the attachment *closest to the end* of the pipeline and runs
exactly that stage, so work in flight finishes before more is started.
Ordering by age instead advanced everything in lockstep, and a poll of
thirteen documents delivered none of them. A crash
...
it reaches the database. IBANs and email addresses are recognised anywhere because they carry their own
structure; national identity numbers, cards and phone numbers only beside a
label, because a company registration number is an extracted field and looks
...
Five of the seven phases spend no tokens at all. The two that do are bounded by
...
21% less and 37% faster — an indication at n=3, not yet an eval. Full breakdown
...
| generated by Gemini, ChatGPT and Grok | 30        | certificates, board resolutions, leases, addenda, an NDA, an SLA |
| collected from TED, unmodified        | 100       | real EU procurement notices, five languages, 1,719 pages         |
